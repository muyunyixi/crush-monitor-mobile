...
function paint(ctx, conversation, options) {
  ...
  let height=130+entries.reduce((n,e)=>n+e.height+12,0)+(options.includeAnalysis?112:0);
  ...
  ctx.setFillStyle('#26332f');ctx.setFontSize(19);ctx.fillText(options.anonymous?'聊天记录':conversation.title,PAD,36);
  ...
  if(options.includeAnalysis){
    ...
    ctx.setFillStyle('#fff');ctx.fillRect(PAD,y,WIDTH-2*PAD,88);
    ctx.setFillStyle('#356f5d');ctx.setFontSize(15);ctx.fillText(`好感信号 ${affinity===null?'—':affinity} /100`,PAD+14,y+31);
    ctx.setFillStyle('#718078');ctx.setFontSize(12);ctx.fillText(`下一步：${conversation.analysis?.overview?.action||'暂无分析'}`,PAD+14,y+60);y+=104;
  }
