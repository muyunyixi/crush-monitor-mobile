...
export function validateConnection(config: ConnectionConfig) {
  ...
  if (!config.key.trim()) throw new Error('请先在右上角设置中填写 TypeSafe API Key。');
  return endpoint;
}
export async function requestAnalysis(job: AnalysisRequest, config: ConnectionConfig, signal: AbortSignal): Promise<AnalysisResponse> {
  const endpoint = validateConnection(config);
  let response: Response;
  try {
    response = await fetch(endpoint, {
      method: 'POST', headers: {'Content-Type':'application/json', Authorization:`Bearer ${config.key.trim()}`},
